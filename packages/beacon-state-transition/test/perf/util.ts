...
function syncPubkeys(): {pubkey2index: PubkeyIndexMap; index2pubkey: PublicKey[]; pubkeys: Uint8Array[]} {
  ...
  return {pubkeys, pubkey2index, index2pubkey};
}
...
export function generatePerfTestCachedStateAltair(opts?: {
  ...
  if (!altairCachedState23637) {
    const state = origState.clone();
    state.slot -= 1;
    altairCachedState23637 = allForks.createCachedBeaconState(config, state, {
      pubkey2index,
      index2pubkey,
      skipSyncPubkeys: true,
    });
